...
- `mergedata.txt` and `summarydata.txt` is generated from processing
the HAR data with `run_analysis.R` R script
- `mergedata.txt` filters all the average and std deviation observations
from test and training data
- `summarydata.txt` summarizes the mean values of the merged data. There
are 180 rows of observations (30 subjects x 6 activities).

This code book summarizes the data fields in `mergedata.txt` and `summarydata.txt`
